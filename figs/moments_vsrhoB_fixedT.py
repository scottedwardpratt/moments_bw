...
rhoB=[0.143511,0.0942362,0.0525105,0.0373777,0.0255775,0.0159063,0.00497411]
colors=['r','g','b','k']
n=0
stderrK=[]
stderrS=[]
...
for tag in tags:
    ...
    for i in range(nroots):
        ...
        file="../moments_rhoB/rhoB"+str(i)+tag+".dat";
    ...
    plt.errorbar(rhoB,Ssigma_avg,stderrS[n],linestyle='-',linewidth=2,color=colors[n],markersize=8, marker='s', markerfacecolor=None, markeredgecolor=None,label=tag+': $C_3/C_1$')
    plt.errorbar(rhoB,Ksigma2_avg,stderrK[n],linestyle='--',linewidth=2,color=colors[n],markersize=10, marker='^', markerfacecolor=None, markeredgecolor=None,label=tag+': $C_4/C_2$')

    n+=1

ax.tick_params(axis='both', which='major', labelsize=14)
ax.set_xticks(np.arange(0,.15,.03), minor=False)
ax.set_xticklabels(np.arange(0,.15,.03), minor=False, family='serif')
ax.set_xticks(np.arange(0,.15,.03), minor=True)
...
plt.xlabel('$\\rho_B$',fontsize=18 , weight='normal')
plt.ylabel('$S\sigma$,  $\kappa\sigma^2$', fontsize=24, weight='normal')
plt.savefig('moments_bw_vsrhoB_fixedT_new.pdf',format='pdf')
os.system('xdg-open moments_bw_vsrhoB_fixedT_new.pdf')
