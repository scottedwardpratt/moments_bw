...
&=&\frac{c_n}{c_m}.
\end{eqnarray}
Note that this suggests that results might be independent of the number of patches, and thus the overall volume, but they might still depend on the volume of an individual path.

Unfortunately, the discussion above is misleading once one considers $C_n$ for $n\ge 4$. In that case the cumulant for a single patche is
\begin{eqnarray}
c_4&=&\langle(Q-\bar{Q})^4\rangle-3\langle(Q-\bar{Q})^2\rangle^2. 
\end{eqnarray}
If one knew the variance, $\langle(Q-\bar{Q})^2\rangle$, for a single patch, the arguments above would be valid. Unfortunately, one typically calculates the variance from a range of systems with different number of patches. ......................
